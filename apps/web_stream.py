...
AVAILABLE_MODELS = {
    "local": {
        "id": "./models/phuonganh-tts-v2",
        "name": "phuonganh-tts-v2 (Local)",
        "desc": "Local model - Offline mode"
    }
}
...
def load_model_instance(model_key):
    ...
    try:
        new_tts = PhuongAnh(
            ...
            backbone_device="cpu", 
            codec_repo="./models/neucodec-onnx-decoder-int8", 
            codec_device="cpu" 
        )
        tts = new_tts
...
try:
    with open(os.path.join(os.path.dirname(os.path.dirname(__file__)), "client", "client.html"), "r", encoding="utf-8") as f:
        HTML_CONTENT = f.read()
    HTML_CONTENT = HTML_CONTENT.replace("phuonganh-tts Stream", "phuonganh-tts GGUF (CPU)")
...
from phuonganh_utils.url_extract import extract_text_from_url
