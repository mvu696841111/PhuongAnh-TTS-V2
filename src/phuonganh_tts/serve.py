...
logger = logging.getLogger("PhuongAnh.Serve")
...
def run_server(args: argparse.Namespace) -> None:
    ...
    """
    logger.info(f"🚀 Starting phuonganh-tts Remote Server...")
    ...
    logger.info(f"   from phuonganh_tts import PhuongAnh")
    logger.info(f"   tts = PhuongAnh(mode='remote', api_base='{sdk_url}/v1', model_name='{args.model_name}')")
    logger.info("")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="phuonganh-tts Remote Server CLI")
    parser.add_argument("--model", type=str, default="./models/phuonganh-tts-v2", help="HuggingFace model ID or local path")
    parser.add_argument("--model-name", type=str, default="phuonganh-tts-v2", help="Model name for API mapping")
    ...
    if args.model != "./models/phuonganh-tts-v2" and args.model_name == "phuonganh-tts-v2":
        args.model_name = args.model
    
    # Check if lmdeploy is installed
    if not check_command("lmdeploy"):
        logger.error("❌ 'lmdeploy' not found!")
        logger.error("   Please install it using: pip install phuonganh-tts[gpu]")
        sys.exit(1)
