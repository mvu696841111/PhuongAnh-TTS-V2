...
logger = logging.getLogger("PhuongAnh.Remote")

class RemotePhuongAnhTTS(BasePhuongAnhTTS):
    """
    Client for phuonganh-tts running on a remote LMDeploy server.
    """

    def __init__(
        self,
        api_base: str = "http://localhost:23333/v1",
        model_name: str = "./models/phuonganh-tts-v2",
        ...
        # Only local model uses the full chat-format prompt
        self.use_chat_format = model_name.rstrip("/").endswith("phuonganh-tts-v2") or "./models/phuonganh-tts-v2" in model_name
    ...
    def cleanup_memory(self):
        """No-op for remote API — no local GPU memory to release."""
        pass
