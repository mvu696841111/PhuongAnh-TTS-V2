...
logger = logging.getLogger("PhuongAnh.XPU")

class XPUPhuongAnhTTS(PhuongAnhTTS):
    """
    XPU (Intel Arc GPU) optimized implementation of phuonganh-tts.
    Uses native PyTorch XPU backend with bfloat16 and specialized compilation/warmup.
    """

    def __init__(
        self,
        backbone_repo="Nemmer/phuonganh-tts-v2",
        backbone_device="xpu", # Forced default
        codec_repo="Nemmer/neucodec-onnx-decoder-int8",
        codec_device="xpu",    # Forced default
        hf_token=None,
    ):
        # Ensure we are strictly on XPU
        if backbone_device != "xpu":
            logger.warning("XPUPhuongAnhTTS initialized with non-xpu device. Forcing 'xpu'.")
            backbone_device = "xpu"
        if codec_device != "xpu":
            codec_device = "xpu"
    ...
    def _load_backbone(self, backbone_repo, backbone_device, hf_token=None):
        ...
        # Resolve local path for transformers compatibility
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(backbone_repo, token=hf_token, local_files_only=local_files_only())
