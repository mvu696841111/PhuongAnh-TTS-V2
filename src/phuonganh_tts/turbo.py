...
from phuonganh_utils.phonemize_text import phonemize_text, phonemize_batch
from phuonganh_utils.core_utils import split_into_chunks_v2, get_silence_duration_v2
from tqdm import tqdm
import sys

logger = logging.getLogger("PhuongAnh.Turbo")

class BaseTurboPhuongAnhTTS(BasePhuongAnhTTS):
    ...
    def _decode(self, codes_str: str, voice_embedding: np.ndarray) -> np.ndarray:
        ...
        # Trim excessive trailing silence (>1 second)
        audio = self._trim_silence(audio, threshold=0.01, min_silence_samples=24000)
        
        return audio


class TurboGPUPhuongAnhTTS(BaseTurboPhuongAnhTTS):
    def __init__(
        self,
        backbone_repo: str = "Nemmer/phuonganh-tts-v2",
        decoder_repo: str = "Nemmer/neucodec-onnx-decoder-int8",
        decoder_filename: str = "model.onnx",
        encoder_repo: str = "Nemmer/neucodec-onnx-decoder-int8",
        encoder_filename: str = "model.onnx",
        device: str = "cuda",
    ...
    def _load_backbone(self, repo, device, hf_token=None, **kwargs):
        from .utils import resolve_model_path, local_files_only
        
        # Resolve local path for transformers/lmdeploy compatibility
        repo = resolve_model_path(repo)
        ...
        if self.backend == "standard":
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(repo, token=hf_token, local_files_only=local_files_only())
            dtype = torch.bfloat16 if self.device == "cuda" else torch.float32
            self.backbone = AutoModelForCausalLM.from_pretrained(repo, torch_dtype=dtype, token=hf_token, local_files_only=local_files_only()).to(torch.device(self.device))
            self.backbone.eval()
...
class TurboPhuongAnhTTS(BaseTurboPhuongAnhTTS):
    def __init__(
        self,
        backbone_repo: str = "Nemmer/phuonganh-tts-v2",
        backbone_filename: str = "phuonganh-tts-v2-Q4-K-M.gguf",
        decoder_repo: str = "Nemmer/neucodec-onnx-decoder-int8",
        decoder_filename: str = "model.onnx",
        encoder_repo: str = "Nemmer/neucodec-onnx-decoder-int8",
        encoder_filename: str = "model.onnx",
    ...
    def _load_backbone(self, backbone_repo, backbone_filename, device, hf_token=None, **kwargs):
        ...
        if os.path.exists(backbone_repo):
            ...
            if local_files_only():
                ...
                cache_dir = snapshot_download(repo_id=backbone_repo, local_files_only=True)
                import glob as _glob
                import os as _os
                pattern = _os.path.join(cache_dir, backbone_filename)
                matches = _glob.glob(pattern)
                if not matches:
                    raise FileNotFoundError(f"Turbo GGUF not found in cache: {pattern}")
                model_path = matches[0]
                logger.info(f"📦 [Offline] Loaded Turbo GGUF from cache: {model_path}")
            else:
                from huggingface_hub import hf_hub_download
