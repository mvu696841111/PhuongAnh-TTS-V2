...
# Configure logging
logger = logging.getLogger("PhuongAnh.Utils")


def local_files_only() -> bool:
    """Return True when offline mode is active (HF_HUB_OFFLINE=1)."""
    return os.environ.get("HF_HUB_OFFLINE") == "1"


def resolve_model_path(repo_or_path: str) -> str:
    """
    Resolve a model path to absolute path if it's a local directory.
    HuggingFace's from_pretrained requires absolute paths for local directories.
    """
    if not repo_or_path:
        return repo_or_path
...
class NeuCodecOnnx:
    ...
    def from_local(cls, local_path):
        """
        Load ONNX model from a local directory or file.
        
        Args:
            local_path: Path to directory containing model.onnx or path to .onnx file.
        
        Returns:
            NeuCodecOnnx instance.
        """
        from pathlib import Path
        ...
        # Look for .onnx files in directory
