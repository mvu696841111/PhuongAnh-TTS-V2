...
from phuonganh_utils.phonemize_text import phonemize_batch
from phuonganh_utils.core_utils import split_text_into_chunks, join_audio_chunks

logger = logging.getLogger("PhuongAnh.Fast")

class FastPhuongAnhTTS(BasePhuongAnhTTS):
    """
    GPU-optimized phuonganh-tts using LMDeploy TurbomindEngine.
    """
    ...
    def __init__(
        ...
        # Resolve local path for lmdeploy
        ...
        logger.info("✅ FastPhuongAnhTTS with optimizations loaded successfully!")
    ...
    def _load_backbone_lmdeploy(self, repo, memory_util, tp, enable_prefix_caching, quant_policy, hf_token=None):
        ...
        try:
            ...
            raise ImportError(
                "Failed to import `lmdeploy`. Install with: pip install phuonganh-tts[gpu]"
            ) from e
    ...
    def _decode(self, codes_str: str) -> np.ndarray:
        ...
        if not speech_ids:
            raise ValueError(
                ...
                "dẫn đến sai số khi tính toán. Bạn hãy thử chuyển sang dùng phiên bản phuonganh-tts-0.3B nếu vẫn muốn dùng LmDeploy hoặc "
