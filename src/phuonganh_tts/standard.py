...
import gc
import logging
from .base import BasePhuongAnhTTS
from .utils import extract_speech_ids, _linear_overlap_add, normalize_device
from phuonganh_utils.phonemize_text import phonemize_with_dict, phonemize_batch
from phuonganh_utils.core_utils import split_text_into_chunks, join_audio_chunks

logger = logging.getLogger("PhuongAnh.Standard")

class PhuongAnhTTS(BasePhuongAnhTTS):
    """
    Standard phuonganh-tts implementation.
    Supports PyTorch + Transformers backend and GGUF quantized models.
    """

    def __init__(
        self,
        backbone_repo: str = "./models/phuonganh-tts-v2",
        backbone_device: str = "cpu",
        codec_repo: str = "./models/neucodec-onnx-decoder-int8",
        ...
        # Resolve local paths to absolute paths for transformers compatibility
        backbone_path = Path(backbone_repo)
        if backbone_path.exists() and not str(backbone_repo).startswith(("http://", "https://", "hf://")):
            backbone_repo = str(backbone_path.resolve())
        ...
        # Only phuonganh-tts local model uses the full chat-format prompt
        backbone_name = backbone_repo.rstrip("/").split("/")[-1]
        self.use_chat_format = backbone_name == "phuonganh-tts-v2"
    ...
    def close(self) -> None:
        ...
        try:
            ...
            logger.error(f"Error during PhuongAnhTTS closure: {e}")
    ...
    def _load_backbone(self, backbone_repo: str, backbone_device: str, hf_token: Optional[str] = None, gguf_filename: Optional[str] = None) -> None:
        from .utils import local_files_only, resolve_model_path
        
        backbone_device = normalize_device(backbone_device)
        
        # Resolve local path for transformers/llama-cpp compatibility
        backbone_repo = resolve_model_path(backbone_repo)
        ...
        # Check if this is a local GGUF file
        is_local_gguf = is_gguf and Path(backbone_repo).exists()
        ...
        if is_local_gguf:
            ...
            # Find the GGUF file in the local directory
            import glob as _glob
            pattern = str(Path(backbone_repo).glob("*.gguf"))
            matches = _glob.glob(pattern)
            if not matches:
                raise FileNotFoundError(f"GGUF file not found in local path: {pattern}")
            model_path = matches[0]
            logger.info(f"   Loading local GGUF: {model_path}")
            self.backbone = Llama(
                model_path,
                verbose=False,
                n_gpu_layers=-1,
            ...
            from .utils import local_files_only
            if local_files_only():
                ...
                import glob as _glob
                import os as _os
                pattern = _os.path.join(cache_dir, gguf_filename or "*.gguf")
                matches = _glob.glob(pattern)
                if not matches:
                    raise FileNotFoundError(f"GGUF file not found in cache: {pattern}")
                model_path = matches[0]
                self.backbone = Llama(
                    model_path,
                    verbose=False,
            ...
            from .utils import local_files_only
            self.tokenizer = AutoTokenizer.from_pretrained(backbone_repo, token=hf_token, trust_remote_code=True, local_files_only=local_files_only())
            ...
            if os.getenv("PHUONGANH_COMPILE") == "1" and platform.system() == "Linux":
                try:
    ...
    def load_lora_adapter(self, lora_repo_id: str, hf_token: Optional[str] = None) -> bool:
        ...
        try:
            from .utils import local_files_only
            self.backbone = PeftModel.from_pretrained(self.backbone, lora_repo_id, token=hf_token, local_files_only=local_files_only())
    ...
    def infer(self, text: str, ref_audio: Optional[Union[str, Path]] = None, ref_codes=None, ref_text: Optional[str] = None, max_chars: int = 256, silence_p: float = 0.15, crossfade_p: float = 0.0, voice: Optional[Dict[str, Any]] = None, temperature: float = 1.0, top_k: int = 50, skip_normalize: bool = False, apply_watermark: bool = True, **kwargs) -> np.ndarray:
        ...
        if len(chunks) == 1:
            ...
            # Ensure phonemes end with punctuation for proper EOS
    ...
    def infer_batch(self, texts: List[str], ref_audio: Optional[Union[str, Path]] = None, ref_codes=None, ref_text: Optional[str] = None, voice: Optional[Dict[str, Any]] = None, temperature: float = 1.0, top_k: int = 50, skip_normalize: bool = False, apply_watermark: bool = True, **kwargs) -> List[np.ndarray]:
        ...
        # If model is Torch, we can leverage true batch generation
            ...
            for i in range(len(texts)):
                ...
                # Find and truncate at end token to prevent hallucinated tokens
                if speech_end_id in generated_list:
                    end_pos = generated_list.index(speech_end_id)
                    generated_list = generated_list[:end_pos]
                
                output_str = self.tokenizer.decode(generated_list, add_special_tokens=False)
                wav = self._decode(output_str)
                if apply_watermark:
                    wav = self._apply_watermark(wav)
                all_wavs.append(wav)
    ...
    def _infer_torch(self, prompt_ids: List[int], temperature: float = 1.0, top_k: int = 50, top_p: float = 0.95) -> str:
        import torch
        prompt_tensor = torch.tensor(prompt_ids).unsqueeze(0).to(self.backbone.device)
        speech_end_id = self.tokenizer.convert_tokens_to_ids("<|SPEECH_GENERATION_END|>")
        
        # Calculate max_new_tokens based on phoneme count in input
        # Count actual phoneme tokens (not special tokens like <|...|>)
        phoneme_count = 0
        for tid in prompt_ids:
            tok = self.tokenizer.decode([tid])
            # Count tokens that look like phonemes (contain letters, not special chars)
            if tok.isalpha() or (len(tok) > 1 and not tok.startswith('<') and tok.strip()):
                phoneme_count += 1
        
        # Speech tokens ~= phoneme_count * 2 (each phoneme becomes ~2 speech tokens)
        # Add 20% buffer for ref audio codes
        min_tokens = max(30, phoneme_count * 2)
        max_new_tokens = min(min_tokens + 200, 2048)  # Cap at 2048
        
        with torch.no_grad():
            output_tokens = self.backbone.generate(
                prompt_tensor,
                max_length=min(len(prompt_ids) + max_new_tokens, self.max_context),
        ...
        generated_ids = output_tokens[0, input_length:].cpu()
    ...
    def _infer_ggml(self, ref_codes: Any, ref_phonemes: str, chunk_phonemes: str, temperature: float = 1.0, top_k: int = 50, emotion_tag: Optional[str] = None) -> str:
        ...
        # Ensure chunk ends with punctuation for proper EOS
        chunk_phonemes = chunk_phonemes.strip()
        if chunk_phonemes and chunk_phonemes[-1] not in '.!?':
        ...
        # Estimate reasonable max tokens based on phoneme count
        # Each phoneme produces ~2 speech tokens, add buffer for ref audio
        phoneme_count = len(chunk_phonemes.split()) + len(ref_phonemes.split())
        max_tokens = min(phoneme_count * 3 + 300, 2048)  # Cap at 2048
        
        output = self.backbone(prompt, max_tokens=max_tokens, temperature=max(0.7, temperature), top_k=max(10, top_k), stop=["<|SPEECH_GENERATION_END|>"])
    ...
    def _infer_stream_ggml(self, ref_codes: Any, ref_phonemes: str, chunk_phonemes: str, temperature: float = 1.0, top_k: int = 50, emotion_tag: Optional[str] = None) -> Generator[np.ndarray, None, None]:
        ...
        # Estimate reasonable max tokens
        phoneme_count = len(chunk_phonemes.split()) + len(ref_phonemes.split())
        max_tokens = min(phoneme_count * 3 + 300, 2048)
        
        for item in self.backbone(prompt, max_tokens=max_tokens, temperature=max(0.7, temperature), top_k=max(10, top_k), stop=["<|SPEECH_GENERATION_END|>"], stream=True):
    ...
    def cleanup_memory(self):
        """Release GPU memory after synthesis."""
        import gc
        try:
            import torch
