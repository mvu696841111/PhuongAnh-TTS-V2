...
# Configure logging
logger = logging.getLogger("PhuongAnh")

class BasePhuongAnhTTS(ABC):
    """
    Abstract base class for phuonganh-tts implementations.
    ...
    def _load_codec(self, codec_repo: str, codec_device: str) -> None:
        ...
        local_path = Path(codec_repo)
        is_local_path = local_path.exists()

        if any(x in codec_repo.lower() for x in ["onnx", "phuonganh-codec"]) or codec_repo == "neuphonic/neucodec-onnx-decoder-int8" or is_local_path:
            if codec_device != "cpu":
                logger.warning("⚠️ ONNX decoder only runs on CPU. Ignoring device selection.")
            try:
                from .utils import NeuCodecOnnx
                if is_local_path:
                    self.codec = NeuCodecOnnx.from_local(local_path)
                else:
                ...
                try:
                    ...
                    if is_local_path:
                        # Load ONNX file directly from local path
                        # Try common ONNX filenames
                        for onnx_name in ["model.onnx", "decoder.onnx", "neucodec.onnx"]:
                            onnx_path = local_path / onnx_name
                            if onnx_path.exists():
                                self.codec = NeuCodecOnnxDecoder(str(onnx_path))
                                self._is_onnx_codec = True
                                return
                        # Fallback: find any .onnx file
                        onnx_files = list(local_path.glob("*.onnx"))
                        if onnx_files:
                            self.codec = NeuCodecOnnxDecoder(str(onnx_files[0]))
                            self._is_onnx_codec = True
                            return
                    self.codec = NeuCodecOnnxDecoder.from_pretrained(codec_repo, local_files_only=local_files_only())
                    self._is_onnx_codec = True
                    return
                except ImportError:
                    raise ImportError(
        ...
        try:
            ...
            # Use cached files when offline (HF_HUB_OFFLINE=1)
            if local_files_only():
                logger.info("📦 [Offline] Loading codec from local cache...")

            if codec_repo == "neuphonic/neucodec":
                self.codec = NeuCodec.from_pretrained(codec_repo, local_files_only=local_files_only())
            elif codec_repo == "neuphonic/distill-neucodec":
                self.codec = DistillNeuCodec.from_pretrained(codec_repo, local_files_only=local_files_only())
            else:
            ...
            raise ImportError(
                ...
                "Or install torch via: pip install phuonganh-tts[gpu]"
            )
    ...
    def _load_voices_from_repo(self, repo_id: str, hf_token: Optional[str] = None) -> None:
        ...
        try:
            ...
            if offline:
                # 2. Already offline, no network allowed — skip silently
                pass
            else:
    ...
    def _decode(self, codes_str: str) -> np.ndarray:
        ...
        """
        # Truncate at end token if present to prevent hallucinated tokens
        codes_str = codes_str.split("<|SPEECH_GENERATION_END|>")[0]
        ...
        # Trim trailing silence only if there's excessive silence (>1 second)
        recon = self._trim_silence(recon, threshold=0.01, min_silence_samples=24000)
        
        return recon

    def _trim_silence(self, wav: np.ndarray, threshold: float = 0.01, min_silence_samples: int = 24000) -> np.ndarray:
        """
        Trim trailing silence/low-energy frames from audio.
        
        Args:
            wav: Audio waveform
            threshold: Energy threshold (relative to max amplitude)
            min_silence_samples: Minimum samples of silence to trigger trim
        
        Returns:
            Trimmed audio waveform
        """
        if len(wav) < min_silence_samples * 2:
            return wav
        
        # Calculate energy (RMS) in short windows
        hop = 480  # ~20ms at 24kHz
        ...
        if max_energy < 0.005:
            return wav  # Audio is too quiet
        
        threshold_abs = max_energy * threshold
        
        # Find the last sample that's above threshold
        last_valid_idx = len(wav) - 1
        for i in range(len(wav) - 1, -1, -1):
            if abs(wav[i]) > threshold_abs:
                last_valid_idx = i
                break
        
        # Only trim if we have significant trailing silence
        trailing_samples = len(wav) - last_valid_idx - 1
        if trailing_samples > min_silence_samples:
            # Keep a buffer of 0.5 seconds
            trim_to = min(last_valid_idx + 12000 + 1, len(wav))
            wav = wav[:trim_to]
    ...
    def _format_prompt(
        ...
        Args:
            use_chat_format: If True, wraps the prompt with chat-style user/assistant
                             tokens (used by phuonganh-tts GPU model). If False (default),
    ...
    def __enter__(self) -> 'BasePhuongAnhTTS':
        return self
