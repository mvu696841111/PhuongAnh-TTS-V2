...
import numpy as np
import torch
from phuonganh_tts.standard import PhuongAnhTTS
...
def test_phuonganh_tts_infer_with_voice_preset(mock_tts_instance):
    with patch("phuonganh_utils.phonemize_text.phonemize_with_dict", return_value="phonemes"):
        audio = mock_tts_instance.infer("Xin chào", voice=mock_tts_instance.get_preset_voice("test_voice"))
        assert isinstance(audio, np.ndarray)
        assert len(audio) == 4800

def test_phuonganh_tts_infer_batch(mock_tts_instance):
    mock_tts_instance._is_quantized_model = False
    texts = ["Text 1", "Text 2"]
    with patch("phuonganh_utils.phonemize_text.phonemize_batch", return_value=["p1", "p2"]), \
...
@patch("phuonganh_tts.standard.BasePhuongAnhTTS._load_codec")
@patch.object(PhuongAnhTTS, '_warmup_model')
def test_phuonganh_tts_gguf_init(mock_warmup, mock_codec, mock_llama):
    mock_llama_instance = MagicMock()
    mock_llama.return_value = mock_llama_instance

    tts = PhuongAnhTTS(backbone_repo="dummy-gguf", backbone_device="cpu")
