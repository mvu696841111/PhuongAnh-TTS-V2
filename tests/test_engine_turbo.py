...
from phuonganh_tts.turbo import TurboPhuongAnhTTS, TurboGPUPhuongAnhTTS
...
def test_turbo_gguf_init(mock_hf, mock_llama, mock_ort):
    tts = TurboPhuongAnhTTS(backbone_repo="dummy", device="cpu")
...
def test_turbo_gguf_infer(mock_hf, mock_llama, mock_ort, mock_onnx_session, mock_llama_instance):
    ...
    with patch("phuonganh_utils.phonemize_text.phonemize_text", return_value="p-h-o-n-e-m-e-s"):
...
def test_turbo_gpu_standard_init(mock_model, mock_tokenizer, mock_hf, mock_ort):
    ...
    tts = TurboGPUPhuongAnhTTS(backbone_repo="dummy", device="cuda", backend="standard")
...
def test_turbo_gpu_lmdeploy_init(mock_pipeline, mock_hf, mock_ort):
    ...
    tts = TurboGPUPhuongAnhTTS(backbone_repo="dummy", device="cuda", backend="lmdeploy")
...
def test_turbo_gpu_infer(mock_model, mock_tokenizer, mock_hf, mock_ort, mock_onnx_session):
    ...
    tts = TurboGPUPhuongAnhTTS(backbone_repo="dummy", device="cpu", backend="standard")
    tts._preset_voices = {"test": {"codes": np.zeros(128), "text": "test"}}
