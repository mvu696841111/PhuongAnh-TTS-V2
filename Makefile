...
help:
	...
	@echo "  make run        - run Gradio UI (auto-detects offline when models are cached)"
	@echo "  make run-online - run with network enabled (force online mode)"
...
stream:
	uv run --frozen phuonganh-stream
...
docker-build-serve:
	docker build -t ghcr.io/mvu696841111/phuonganh-tts:serve -f docker/Dockerfile.serve .

docker-push-serve:
	docker push ghcr.io/mvu696841111/phuonganh-tts:serve
