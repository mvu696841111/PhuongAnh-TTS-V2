# 🦜 Hướng dẫn Fine-tune phuonganh-tts (LoRA)

Thư mục này chứa toàn bộ công cụ cần thiết để bạn huấn luyện (fine-tune) mô hình phuonganh-tts với giọng nói của riêng mình bằng phương pháp **LoRA (Low-Rank Adaptation)**.
...
- `model`: Chọn base model (vd: `Nemmer/phuonganh-tts-v2`).
...
Nếu bạn không quen sử dụng script console, chúng tôi cung cấp file Notebook `finetune_phuonganh-tts.ipynb`. File này đã tích hợp sẵn mọi bước từ chuẩn bị đến huấn luyện, cực kỳ dễ theo dõi trên Google Colab hoặc máy cục bộ.
...
- ✅ Người dùng chỉ cần: `tts = PhuongAnh(backbone_repo="./models/phuonganh-tts-v2")`
...
tts = PhuongAnh(backbone_repo="your-username/your-model")
